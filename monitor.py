...
def main() -> None:
    ...
    for domain, records in records_by_domain.items():
        ...
        new_messages = fetch_replies_for_sar(
            service, latest_record, existing_ids, user_email=email, verbose=args.verbose
        )
                    ...
                    try:
                        ...
                        if schema_result:
                            ...
                            cat = AttachmentCatalog(
                                ...
                                categories=[
                                    c["name"]
                                    for c in schema_result.get("categories", [])
                                ],
                            ...
                            if args.verbose:
                                print(
                                    f"  [{state.company_name}] inline data schema: "
            ...
            if set(reply.tags) & _VERIFY_TAGS:
                ...
                if portal_url:
                    try:
                        ...
                        if args.verbose:
                            print(
                                f"  [{state.company_name}] portal verification failed: {exc}"
                            )
    ...
    if args.reprocess:
        ...
        if n_changed and not args.dry_run:
            ...
            save_state(
                email, sp_states2, path=data_dir / "subprocessor_reply_state.json"
            )
        if args.dry_run:
    ...
    if args.draft_backfill:
        ...
        if n_sp:
            save_state(
                email, sp_states, path=data_dir / "subprocessor_reply_state.json"
            )
    ...
    if sp_count and args.verbose:
        print(
            f"[subprocessor] {sp_count} new reply(ies) for subprocessor disclosure requests\n"
        )
    ...
    _auto_download_data_links(
        email, states, api_key, verbose=args.verbose, data_dir=data_dir
    )
...
def _monitor_subprocessor_requests(
    service, email: str, account: str, *, data_dir: Path
) -> int:
