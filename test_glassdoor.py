...
def info(msg: str, indent: int = 2) -> None:
    prefix = " " * (indent + 4)
    for line in textwrap.wrap(msg, width=68, initial_indent="  · ", subsequent_indent=prefix):
        print(line)
...
def step2_reclassify(glassdoor: dict) -> None:
    ...
    for i, r in enumerate(glassdoor.get("replies", []), 1):
        ...
        if is_noise:
            warn("NON_GDPR pre-pass FIRES → short-circuits classifier (newsletter/marketing)")
        else:
        ...
        field("data_link",   result.extracted.get("data_link") or "(not found in snippet)")
        if result.extracted.get("data_link"):
            ok("URL found in snippet!")
        elif "DATA_PROVIDED_LINK" in result.tags:
            warn("DATA_PROVIDED_LINK tag correct — but URL not in snippet (likely in body)")
        print()


# ─── Step 3: Re-fetch email body from Gmail ───────────────────────────────────

def step3_body_extraction(glassdoor: dict, skip_gmail: bool, account: str = ACCOUNT) -> str | None:
    ...
    if skip_gmail:
        ...
        result = _extract(target.get("from", ""), target.get("subject", ""), target.get("snippet", ""))
    ...
    body_result = _extract(
        target.get("from", ""), target.get("subject", ""), target.get("snippet", ""), body
    )

    print(f"  {BOLD}URL extraction comparison:{RESET}")
    field("Snippet-only data_link", snippet_result.get("data_link") or "(empty — this was the bug)")
    ...
    if body_result.get("data_link"):
        ...
        warn("URL still not found in body text — checking raw body for glassdoor.com URLs…")
        import re
    ...
    field("Full classify data_link", full_result.extracted.get("data_link") or "(empty)")
...
def step4_download(data_link: str | None, skip_download: bool) -> dict | None:
    ...
    if not data_link:
        ...
        info("To get the link: run 'python monitor.py --account traderm1620@gmail.com' first, "
    ...
    try:
        ...
        field("Content-Length", f"{content_length:,} bytes ({content_length/1024:.1f} KB)" if content_length else "(not provided)")
    except Exception as exc:
    ...
    if head.status_code in (401, 403, 404, 410):
        ...
        info("Glassdoor's download links are time-limited (typically 7 days). "
    ...
    if content_length > MAX:
        warn(f"File is {content_length / 1048576:.1f} MB — exceeds 100 MB auto-download limit")
    ...
    ok(f"Downloaded successfully!")
    field("Saved to",    result.catalog.path)
    field("Size",        f"{result.catalog.size_bytes:,} bytes ({result.catalog.size_bytes/1024:.1f} KB)")
...
def step5_catalog_analysis(catalog: dict | None, glassdoor: dict) -> None:
    ...
    if catalog is None:
        ...
        info("  • Data categories inferred from filenames (Search History, Profile Data, etc.)")
        ...
        warn("Category matching is filename-based regex — may miss Glassdoor-specific names")
        warn("No field-level schema (type/example/description) — dataowners.org shows this level of detail")
        warn("Category→file association uses substring matching, may have false positives")
        return

    ok(f"Catalog loaded — {len(catalog.get('files', []))} files, "
    ...
    if uncategorised:
        ...
        for fname in uncategorised:
            print(f"      · {fname}")
        info("Fix: add Glassdoor-specific patterns to _CATEGORY_HINTS in attachment_handler.py")
    else:
        ok("All files matched to at least one category")

    warn("No field-level schema (column names, types, example values)")
    info("dataowners.org shows type + example + description per field. "
    ...
    info("Could add: row count for CSVs, key count for JSONs, to give a sense of data volume.")

    if any(f.get("file_type") == "json" for f in catalog.get("files", [])):
        ok("JSON files detected — can be introspected for schema keys")
        info("Next step: open each JSON in the received/ folder, print top-level keys, "
    ...
    if any(f.get("file_type") == "csv" for f in catalog.get("files", [])):
        ...
        info("Next step: read first row of each CSV, display column names "
...
def main() -> None:
    ...
    parser.add_argument("--skip-gmail",    action="store_true", help="Skip live Gmail fetch")
    parser.add_argument("--skip-download", action="store_true", help="Skip HTTP download")
