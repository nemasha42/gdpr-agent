...
class TestBouncePermanent:
    ...
    def test_postmaster_550(self):
        result = classify(
            msg(
                from_addr="postmaster@reflexivity.com",
    ...
    def test_no_such_user(self):
        result = classify(
            msg(from_addr="mailer-daemon@smtp.example.com", snippet="No such user here")
        )
        assert "BOUNCE_PERMANENT" in result.tags
...
class TestAutoAcknowledge:
    ...
    def test_snippet_received_your_request(self):
        result = classify(
            msg(snippet="We received your request and will respond within 30 days")
        )
        assert "AUTO_ACKNOWLEDGE" in result.tags
...
class TestOutOfOffice:
    ...
    def test_automatic_reply_snippet(self):
        result = classify(
            msg(snippet="I am away on annual leave and will return on Monday")
        )
        assert "OUT_OF_OFFICE" in result.tags
...
class TestIdentityRequired:
    def test_proof_of_identity(self):
        result = classify(
            msg(
                snippet="please provide proof of identity before we can process your request"
            )
        )
    ...
    def test_government_issued_id(self):
        result = classify(
            msg(snippet="We need a government-issued photo ID to verify your identity")
        )
        assert "IDENTITY_REQUIRED" in result.tags

    def test_passport_copy(self):
        result = classify(
            msg(snippet="Please send a copy of your passport for verification")
        )
        assert "IDENTITY_REQUIRED" in result.tags
...
class TestMoreInfoRequired:
    def test_please_clarify(self):
        result = classify(
            msg(
                snippet="Please clarify your request — we cannot identify you in our systems"
            )
        )
    ...
    def test_unable_to_locate(self):
        result = classify(
            msg(snippet="We are unable to locate any record matching your details")
        )
        assert "MORE_INFO_REQUIRED" in result.tags
...
class TestWrongChannel:
    ...
    def test_german_nicht_gelesen(self):
        result = classify(
            msg(
                snippet="Diese Adresse wird nicht gelesen. Bitte nutzen Sie das Formular."
            )
        )
        assert "WRONG_CHANNEL" in result.tags
    ...
    def test_privacy_portal_mention(self):
        result = classify(
            msg(snippet="Please submit your request via our privacy portal")
        )
        assert "WRONG_CHANNEL" in result.tags
    ...
    def test_request_marked_as_resolved(self):
        result = classify(
            msg(
                snippet="Your request has been marked as resolved. If you need further help, contact us."
            )
        )
        assert "WRONG_CHANNEL" in result.tags
    ...
    def test_case_closed(self):
        result = classify(
            msg(snippet="Your case has been closed. Thank you for contacting support.")
        )
        assert "WRONG_CHANNEL" in result.tags
...
class TestRequestAccepted:
    def test_confirmed_and_begin_gathering(self):
        result = classify(
            msg(
                snippet="We have confirmed your request and will begin gathering your data"
            )
        )
    ...
    def test_start_of_request_subject(self):
        result = classify(
            msg(subject="Start of Your Request to Access Your Personal Data")
        )
        assert "REQUEST_ACCEPTED" in result.tags
...
class TestDataProvided:
    ...
    def test_data_attachment(self):
        result = classify(
            msg(has_attachment=True, snippet="Please find your data attached")
        )
    ...
    def test_data_portal(self):
        result = classify(
            msg(
                snippet="You can access your data via our self-service account management page"
            )
        )
...
class TestDenial:
    ...
    def test_no_data_held(self):
        result = classify(
            msg(snippet="We do not hold any data or records about you in our systems")
        )
    ...
    def test_not_gdpr_applicable(self):
        result = classify(
            msg(
                snippet="GDPR does not apply to our organisation as we are not based in the EU or UK"
            )
        )
    ...
    def test_fulfilled_deletion(self):
        result = classify(
            msg(
                snippet="Your account has been removed and your data has been deleted from our systems"
            )
        )
...
class TestExtended:
    def test_extended_three_months(self):
        result = classify(
            msg(
                snippet="Due to the complexity we require more time — up to three months"
            )
        )
...
class TestHumanReview:
    ...
    def test_human_review_no_api_key(self):
        ...
        result = classify(
            msg(snippet="Something completely unclassifiable xyz123"), api_key=None
        )
        assert "HUMAN_REVIEW" in result.tags
...
class TestJunkURLFiltering:
    ...
    def test_data_export_not_junk(self):
        ...
        assert (
            _is_junk_url("https://example.com/data-export/download?token=abc") is False
        )
...
class TestDraftTone:
    def test_closure_draft_mentions_follow_instructions(self):
        ...
        mock_response = MagicMock()
        mock_response.content = [
            MagicMock(text="I will follow the portal instructions.")
        ]
        mock_response.usage.input_tokens = 100
        ...
        with patch("reply_monitor.classifier.anthropic") as mock_anthropic:
            ...
            assert (
                "argue" not in prompt_text.lower()
                or "do not argue" in prompt_text.lower()
            )
    ...
    def test_standard_redirect_draft_no_closure_context(self):
        ...
        mock_response = MagicMock()
        mock_response.content = [
            MagicMock(text="Please clarify the appropriate channel.")
        ]
        mock_response.usage.input_tokens = 100
        mock_response.usage.output_tokens = 30
        ...
        with patch("reply_monitor.classifier.anthropic") as mock_anthropic:
            ...
            assert (
                "closed" not in prompt_text.lower()
                or "follow their portal" not in prompt_text.lower()
            )
