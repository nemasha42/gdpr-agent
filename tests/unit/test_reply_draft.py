...
class TestGenerateReplyDraft:
    def test_returns_empty_without_api_key(self):
        result = generate_reply_draft("body text", ["WRONG_CHANNEL"], "Acme Corp", api_key=None)
        assert result == ""

    def test_returns_empty_for_non_action_tags(self):
        result = generate_reply_draft(
            "body text", ["AUTO_ACKNOWLEDGE", "REQUEST_ACCEPTED"], "Acme Corp", api_key="sk-test"
        )
        assert result == ""
    ...
    def test_returns_draft_on_success(self):
        ...
        mock_response.content = [MagicMock(text="  Please specify the correct channel.\n  ")]
        mock_response.usage.input_tokens = 100
        mock_response.usage.output_tokens = 50

        with patch("anthropic.Anthropic") as mock_cls, \
             patch("contact_resolver.cost_tracker.record_llm_call") as mock_record:
    ...
    def test_draft_generated_for_each_action_tag(self):
        ...
        for tag in _ACTION_DRAFT_TAGS:
            with patch("anthropic.Anthropic") as mock_cls, \
                 patch("contact_resolver.cost_tracker.record_llm_call"):
                mock_cls.return_value.messages.create.return_value = mock_response
                result = generate_reply_draft("body", [tag], "TestCo", api_key="sk-test")
