...
def scan_folder(domain: str):
    ...
    if not received_dir.exists():
        return redirect(
            url_for("data_card", domain=domain, account=account, scan_error="no_files")
        )
...
def download_data(domain: str):
    ...
    return redirect(
        url_for("data_card", domain=domain, account=account, download_error=error)
    )
...
def refresh():
    ...
    if account:
        ...
        try:
            _run_subprocessor_monitor_for_account(
                account, _service=_service, _email=_email
            )
        except Exception as exc:
...
def _run_monitor_for_account(account: str):
    ...
    for domain, records in records_by_domain.items():
        ...
        new_messages = fetch_replies_for_sar(
            service, latest_record, existing_ids, user_email=email
        )
            ...
            if (
                ...
                try:
                    ...
                    if result.ok:
                        ...
                        needs_save = True
                        print(
                            f"[auto-download] {domain}: ✓ {len(result.catalog.files)} files, "
                            f"{len(result.catalog.schema)} schema categories"
                        )
                    else:
                        print(
                            f"[auto-download] {domain}: ✗ {result.error or 'expired' if result.expired else result.error}"
                        )
                except Exception as exc:
...
def _auto_analyze_inline_data(account: str, states: dict, api_key: str | None) -> None:
    ...
    for domain, state in states.items():
        for reply in state.replies:
            if "DATA_PROVIDED_INLINE" in reply.tags and not reply.attachment_catalog:
...
from dashboard.scan_state import (
    load_scan_state,
    save_scan_state,
    get_all_accounts as _get_scan_accounts,
)
...
def _load_companies_db() -> dict:
    try:
        return _json.loads(_COMPANIES_PATH.read_text()).get(
            "companies", _json.loads(_COMPANIES_PATH.read_text())
        )
    except Exception:
        return {}
